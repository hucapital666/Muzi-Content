...
import { Loader2, Copy, Check, Sparkles, Image as ImageIcon, Video, FileText, KeyRound, Download, Upload, X, LogOut, User, Trash2, Search } from 'lucide-react';
import { auth, signInWithGoogle, logOut, getUserApiKey, saveUserApiKey } from './firebase';
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      setIsAuthReady(true);
      if (currentUser) {
        const storedKey = await getUserApiKey(currentUser.uid);
        if (storedKey) {
  ...
  useEffect(() => {
    const checkApiKey = async () => {
      if (window.aistudio?.hasSelectedApiKey) {
        ...
        setHasApiKey(!!localStorage.getItem('gemini_api_key') || !!customApiKey);
      }
    };
    checkApiKey();
  }, [customApiKey]);
  ...
  const handleSaveCustomApiKey = async (key: string) => {
    ...
    if (user && trimmedKey) {
      await saveUserApiKey(user.uid, trimmedKey);
    }
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {user ? (
                ...
                <button
                  onClick={handleLogin}
                  className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-300 hover:bg-gray-50 rounded-lg text-sm font-medium text-gray-700 transition-colors shadow-sm"
                >
              ...
              >
                ...
                {hasApiKey ? 'Đã kết nối API Key' : 'Cài đặt API Key'}
              </button>
            </div>
          </div>
          ...
          {activeTab === 'image' && (
            <div className="space-y-6 pb-8">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-6">
                ...
                <div className="flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Lọc phân cảnh</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-shadow"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 mt-6">
                {imagePrompts.map((prompt, index) => {
                  if (imageFilter && !prompt.toLowerCase().includes(imageFilter.toLowerCase())) {
                    return null;
                  }
                  return (
                    <div key={index} className="p-4 border border-gray-200 rounded-xl bg-white shadow-sm">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider">Phân cảnh {index + 1}</h3>
                        <button
                          onClick={() => {
                          ...
                          disabled={!prompt}
                          className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-indigo-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Copy Prompt"
                        >
            ...
            <div className="space-y-6 pb-8">
              {!hasApiKey && (
                <div className="bg-amber-50 p-4 rounded-lg border border-amber-200 mb-6 flex flex-col items-start gap-3">
                  <p className="text-sm text-amber-800">
                    Bạn cần <strong>Cài đặt API Key</strong> ở góc trên để sử dụng tính năng tạo Video (yêu cầu API Key có hỗ trợ Veo).
                  </p>
                </div>
              )}
              ...
              <div className="space-y-6 mt-6">
                {videoPrompts.map((prompt, index) => {
                  if (videoFilter && !prompt.toLowerCase().includes(videoFilter.toLowerCase())) {
                    return null;
                  }
                  return (
                    <div key={index} className="p-4 border border-gray-200 rounded-xl bg-white shadow-sm">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 relative">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Để sử dụng tính năng tạo ảnh và video, bạn cần cung cấp Gemini API Key. Key này sẽ được lưu an toàn trên trình duyệt của bạn (hoặc đồng bộ nếu bạn đã đăng nhập).
              <br/><br/>
              Bạn có thể lấy API Key tại: <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline">Google AI Studio</a>
            </p>
